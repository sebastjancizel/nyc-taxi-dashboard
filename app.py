...
def create_figure_geomap(pickup_counts, zoom=10, center={"lat": 40.7, "lon": -73.99}):
    ...
    # Custom tool-tip
    hovertemplate = "<br>Zone: %{location}" "<br>Number of trips: %{customdata:.3s}"
...
def create_daily_plot(daily):
    ...
    weekly_fig.update_yaxes(
        zeroline=True, showgrid=True, gridwidth=0.5, gridcolor="#d5d5d5"
    )
...
def create_destination_table(destinations):
    ...
    for zone, share in destinations.items():
        if zone != "Other":
            row = html.Tr(
                [
                    html.Td(f"{zone}", style=text_style),
                    html.Td(f"{share/10:.1%}", style=text_style),
                ]
            )
            rows.append(row)
...
def create_heatmap(heatmap):
    ...
    heatmap_fig = px.imshow(
        heatmap, x=list(range(24)), y=DAYS, color_continuous_scale="RdYlBu"
    )

    heatmap_fig.update_layout(CHART_LAYOUT)

    heatmap_fig.update_layout(coloraxis=dict(reversescale=True, showscale=False))

    hovertemplate = "<br>Hour: %{x}" "<br>Day: %{y}" "<br>Price: %{z}"
...
#
map = dbc.Card(
    dbc.CardBody(dcc.Graph(id="nyc_map", figure=chart, style={"height": "80vh"}))
)
                ...
                dbc.CardDeck(
                    [
                        ...
                        build_graph_element(
                            hourly,
                            id="hourly",
                            title="Ride frequency throughout the day",
                            style={"height": "25vh"},
                        ),
                    ],
                    className="mb-3",
...
description = dbc.Modal(
    [
        ...
        dbc.ModalFooter(dbc.Button("Close", id="close-description")),
    ],
    id="description",
    size="xl",
)
...
@app.callback(
    ...
    [Input("description-button", "n_clicks"), Input("close-description", "n_clicks")],
